// ============================================================
// useDefaultBlockStore.ts — 側邊欄「通用預設區塊」管理（可增刪改、持久化）。
//
// 定位：這些區塊為「通用（characterId: null）」基礎招式，不綁角色，可拖到任一泳道。
//   - 首次載入以 constants/defaultBlocks 的 7 個內建區塊為「種子」寫入。
//   - 之後使用者可新增、命名（rename label）、選取、刪除；全程 LocalStorage 持久化。
//   - 顏色固定中性灰（不可改）；label 為唯一可編輯內容。
// 選取狀態（多選拖曳／批量刪除用）不持久化，比照 useSidebarStore 的模板選取。
// ============================================================
...
const STORAGE_KEY = 'wuwa-rotation-default-blocks';
/** 通用預設區塊的中性灰（系統預設、非角色專屬）。 */
export const DEFAULT_BLOCK_COLOR = '#64748B';

/** 正規化為合法 DefaultBlock（防手改 localStorage：強制通用欄位、固定色）。 */
function normalize(b: Partial<DefaultBlock>): DefaultBlock {
  return {
...
function loadFromStorage(): DefaultBlock[] {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw === null) return DEFAULT_BLOCKS.map((b) => normalize(b));
    const parsed = JSON.parse(raw) as Partial<DefaultBlock>[];
    return Array.isArray(parsed) ? parsed.map(normalize) : DEFAULT_BLOCKS.map((b) => normalize(b));
  } catch (e) {
    console.warn('[useDefaultBlockStore] 讀取失敗，改用內建種子', e);
    return DEFAULT_BLOCKS.map((b) => normalize(b));
  }
}

export const useDefaultBlockStore = defineStore('defaultBlocks', () => {
  /** 通用預設區塊清單（種子 + 使用者自訂）；LocalStorage 初始化。 */
  const blocks = ref<DefaultBlock[]>(loadFromStorage());
  ...
  if (localStorage.getItem(STORAGE_KEY) === null) {
    try {
      ...
      console.warn('[useDefaultBlockStore] 種子寫入失敗', e);
    }
  }
  ...
  watch(
    ...
    (v) => {
      try {
        ...
        console.warn('[useDefaultBlockStore] 寫入失敗', e);
      }
    },
    { deep: true },
  );
